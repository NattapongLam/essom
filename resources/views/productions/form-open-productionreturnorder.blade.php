                        ...
                        <div class="row align-items-center">
                            <div class="col-12 col-xl-3 mb-3 mb-xl-0">
                                <h3 class="card-title text-dark mb-0" style="font-weight: 700; font-size: 1.4rem;">
                                    <i class="fas fa-boxes-packing text-indigo mr-2"></i> เอกสารรับคืนจากการเบิก
                                </h3>
                            </div>
                            ...
                            <table class="table table-custom table-hover align-middle table-sm" id="tb_job" style="width: 100%;">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center text-secondary py-3">เลขที่ใบรับคืน</th>
                                    ...
                                    <tr>
                                        <td class="text-center font-weight-bold">
                                            <span class="badge badge-light-indigo px-2.5 py-1.5 rounded">
                                                {{$item->returnorder_status_name}}
                                            </span>
                                        </td>
                                        <td class="text-center font-weight-bold text-secondary">{{\Carbon\Carbon::parse($item->returnorder_hd_date)->format('d/m/Y')}}</td>
                                        <td class="text-center text-primary font-weight-bold">{{$item->returnorder_hd_docuno}}</td>
                                        <td class="text-center text-indigo font-weight-bold">{{$item->ladingorder_hd_docuno}}</td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="{{route('pd-retu.edit',$item->returnorder_hd_id)}}" class="btn btn-sm btn-action-edit shadow-sm" title="แก้ไข">
                                                <i class="fas fa-edit"></i>
                                            </a>                                                   
                                            @else
                                            <a href="javascript:void(0)" class="btn btn-sm btn-action-view shadow-sm" data-toggle="modal" data-target="#modal" onclick="getDataRetu('{{ $item->returnorder_hd_id }}')" title="ดูรายละเอียด">
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-xl">
            <div class="modal-header border-bottom-0 pt-4 px-4">
                <h5 class="modal-title font-weight-bold text-dark"><i class="fas fa-info-circle text-indigo mr-2"></i>รายละเอียดเอกสารรับคืน</h5>
                    ...
                    <table class="table table-custom table-hover align-middle table-sm" style="width: 100%;">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center text-secondary py-2" style="width: 150px;">รหัสสินค้า</th>                                   
                                <th class="text-secondary py-2">ชื่อสินค้า</th>    
                                <th class="text-center text-secondary py-2" style="width: 120px;">หน่วยนับ</th>  
                                <th class="text-right text-secondary py-2" style="width: 140px;">จำนวนเบิก</th>   
                                <th class="text-right text-secondary py-2" style="width: 140px;">จำนวนคืน</th>    
                            </tr>
                        </thead>
...
const getDataRetu = (id) => {
    // เคลียร์ข้อมูลเก่าทุกครั้งก่อนโหลดเพื่อความเรียบร้อย
    $('#tb_list').html('<tr><td colspan="6" class="text-center">กำลังโหลดข้อมูลรายละเอียด...</td></tr>');
        ...
        success: function(data) {
            ...
            if(data.dt && data.dt.length > 0) {
                $.each(data.dt, function(key, item) {
                    let lading_qty = item.ladingorder_dt_qty ? parseFloat(item.ladingorder_dt_qty).toFixed(2) : '0.00';
                    let return_qty = item.returnorder_dt_qty ? parseFloat(item.returnorder_dt_qty).toFixed(2) : '0.00';
                    ...
                    el_list += `    
                        <tr>
                            ...
                            <td class="text-right text-indigo font-weight-bold">${lading_qty}</td>  
                            <td class="text-right text-success font-weight-bold">${return_qty}</td> 
                        </tr>
                    `;
                });
            } else {
                el_list = '<tr><td colspan="6" class="text-center text-danger">ไม่พบข้อมูลรายละเอียดในเอกสารนี้</td></tr>';
            }
            $('#tb_list').html(el_list);
        },
        error: function(xhr, status, error) {
            console.error(error);
            $('#tb_list').html('<tr><td colspan="6" class="text-center text-danger">เกิดข้อผิดพลาดในการดึงข้อมูลจากระบบ</td></tr>');
        }
    });
}  
</script>
