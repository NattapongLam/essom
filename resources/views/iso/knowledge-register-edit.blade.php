    ...
    <form action="{{ route('knowledge-register.update', $record->id) }}" method="POST">
        ...
        <table id="knowledgeTable">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td><input type="text" name="document_code[]" value="{{ $doc->document_code }}"></td>
                    <td><input type="date" name="received_date[]" value="{{ $doc->received_date }}"></td>
                    <td><input type="text" name="doc_title[]" value="{{ $doc->doc_title }}"></td>
        ...
        <div class="actions">
            <a href="{{ route('knowledge-register.index') }}"><button type="button" class="edit">ย้อนกลับ</button></a>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    addRowBtn.addEventListener('click', () => {
        ...
        newRow.innerHTML = `
            <td>${rowCount}</td>
            <td><input type="text" name="document_code[]"></td>
            <td><input type="date" name="received_date[]"></td>
            <td><input type="text" name="doc_title[]"></td>
            <td><button type="button" class="delete">ลบ</button></td>
        `;
