                        ...
                        <div class="row align-items-center g-3">
                            <div class="col-12 col-xl-3 col-lg-12">
                                <h3 class="m-0" style="font-weight: 800; color: var(--text-dark);">
                                    <i class="fas fa-folder-open text-indigo-600 me-2" style="color: var(--indigo-primary);"></i> เอกสาร CAR
                                </h3>
                            </div>
                            ...
                            <div class="col-6 col-xl-1-5 col-md-3">
                                <a href="{{route('car-report.create')}}" class="btn btn-indigo-create text-nowrap">
                    ...
                    <div class="table-responsive">
                        <table class="table table-modern" id="tb_job" style="width:100%">
                            ...
                            <tbody>
                                @foreach ($hd as $item)
                                    @php
                                        // ใช้ฟิลด์วันทีกระทำผิด/วันที่ของเอกสาร CAR (troublemaker_date)
                                        $reportDateValue = $item->troublemaker_date ?? now();
                                        $reportedDate = \Carbon\Carbon::parse($reportDateValue);
                                        $daysPassed = $reportedDate->diffInDays(\Carbon\Carbon::now());
                                        
                                        // กำหนดสีแถบตามเงื่อนไขความเก่าของวันที่
                                    ...
                                    <tr>
                                        <td>
                                            <div class="status-badge-wrapper">
                                                ...
                                                <span class="status-count-pill" title="จำนวนวันที่ผ่านมานับจากวันที่เอกสาร">{{ $daysPassed }} วัน</span>
                                            </div>
                                        </td>
                                        <td class="fw-medium">{{$reportedDate->format('Y/m/d')}}</td>
                                        <td>{{$item->iso_car_refertype}}</td>
                                        <td class="fw-bold text-indigo-dark">{{$item->iso_car_docuno}}</td>
                                        <td>{{$item->problem_by}}</td>
                                        <td><div class="text-truncate" style="max-width: 220px;" title="{{$item->found_bugs}}">{{$item->found_bugs}}</div></td>
                                        <td class="text-center">
                                            <a href="{{route('car-report.edit',$item->iso_car_id)}}" class="btn btn-warning btn-action text-white" title="แก้ไข">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="javascript:void(0)" class="btn btn-danger btn-action" 
                                               onclick="confirmDel('{{ $item->iso_car_docuno }}','{{ $item->iso_car_id }}')" title="ลบ">
