...
<style>
    ...
    /* Monthly Sign-off Checkers Grid */
    .check-date-grid {
        display: grid;
    ...
    <div class="card form-card">
        ...
        <div class="card-header-modern">
            <div class="header-title-block text-center">
                ...
                <h2 class="mt-2 mb-0">การบำรุงรักษาอุปกรณ์ IT (แก้ไขข้อมูล)</h2>
            </div>
        ...
        <!-- Master Global Form Lifecycle Block -->
        <form id="maintenanceForm" method="POST" action="{{ route('computer-records.update', $record->id) }}">
            @csrf
            @method('PUT')
            
            <div class="card-body" style="padding: 2rem 2.5rem;">

                <!-- Core Details Information Input Group Box -->
                <div class="info-input-group">
                    <div class="row">
                        <div class="col-md-4 mb-3 mb-md-0">
                            <label><i class="fas fa-hashtag text-indigo mr-1"></i> For Asset Number</label>
                            <input type="text" class="form-control-modern" name="asset_number" value="{{ $record->asset_number }}" required>
                        </div>
                        <div class="col-md-4 mb-3 mb-md-0">
                            <label><i class="fas fa-user text-indigo mr-1"></i> User Name</label>
                            <input type="text" class="form-control-modern" name="user_name" value="{{ $record->user_name }}" required>
                        </div>
                        <div class="col-md-4">
                            <label><i class="fas fa-calendar-alt text-indigo mr-1"></i> Period</label>
                            <input type="text" class="form-control-modern" name="period" value="{{ $record->period }}" required>
                        </div>
                    </div>
                </div>
                                ...
                                <tr>
                                    ...
                                    @for($m=0;$m<12;$m++)
                                        ...
                                        <td>
                                            ...
                                            <div class="triple-checkbox {{ $val==1?'checked':($val==2?'cross':'') }}" data-name="month_{{ $i }}_{{ $m }}">
                                                {{ $val==1?'✔':($val==2?'✖':'') }}
                                            </div>
                                        </td>
                                    @endfor
                ...
                <div class="step" id="step2">
                    <div class="table-responsive-container">
                        <table class="table-modern text-center m-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="font-weight-bold text-secondary">{{ $valIndex + 1 }}</td>
                                    <td style="text-align:left; padding-left: 15px; color: #1e293b;">{{ $text }}</td>
                                    @for($m=0;$m<12;$m++)
                                        @php $val = $status[$valIndex][$m] ?? 0; @endphp
                                        <td>
                                            <input type="hidden" name="month_{{ $valIndex }}_{{ $m }}" value="{{ $val }}">
                                            <div class="triple-checkbox {{ $val==1?'checked':($val==2?'cross':'') }}" data-name="month_{{ $valIndex }}_{{ $m }}">
                                                {{ $val==1?'✔':($val==2?'✖':'') }}
                                            </div>
                                        </td>
                                    @endfor
                    ...
                    <!-- Monthly Auditor Information Grid System -->
                    <div class="check-date-grid">
                        ...
                        <div class="month-checker-card">
                            ...
                            <input type="text" class="form-control-modern mb-2" name="check_by_{{ $m }}" value="{{ $record->check_by[$m] ?? '' }}" placeholder="ผู้ตรวจ">
                            
                            <label>Date Checked</label>
                            <input type="date" class="form-control-modern" name="date_check_{{ $m }}" value="{{ $record->date_check[$m] ?? '' }}">
                        </div>
                        @endfor
                    </div>

                    <!-- Remarks Textarea Control Block -->
                    <div class="row mt-4">
                        <div class="col-12">
                            <label class="font-weight-bold text-secondary mb-2" style="font-size: 0.9rem;">Remark / หมายเหตุเพิ่มเติม :</label>
                            <textarea class="form-control-modern" name="remark" rows="4" placeholder="กรอกรายละเอียด หรือบันทึกเพิ่มเติมเกี่ยวกับการตรวจสภาพ..." style="resize: vertical;">{{ $record->remark }}</textarea>
                        </div>
                    </div>
                </div>

                <!-- Usage Instruction Guide Box -->
                ...
                <!-- Footer Operations Step Control Panel Bar -->
                <div id="pagination" class="d-flex justify-content-center align-items-center mt-5" style="gap: 15px;">
                    ...
                    <button type="submit" id="submitBtn" class="btn-success-save" style="display:none;"><i class="fas fa-save mr-1"></i> อัปเดตข้อมูล</button>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Triple-state Dynamic Checker Component Control Logic
    ...
    form.addEventListener('submit', function(e) {
        ...
        Swal.fire({
            ...
            text: 'กรุณาตรวจสอบการเปลี่ยนแปลงให้ถูกต้องก่อนกดยืนยัน อัปเดตข้อมูลลงระบบ',
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: '<i class="fas fa-check mr-1"></i> ยืนยันการอัปเดต',
