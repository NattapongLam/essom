...
<div class="form-container">
    ...
    <div class="section-top-fields">
        <div class="row">
            <div class="col-md-4">
                <label>Section:</label>
                <input type="text" name="section[]" value="{{ old('section.0', isset($newObjective) ? $newObjective->section : '') }}" class="form-control" required>
            </div>
            <div class="col-md-4">
                <label>Period:</label>
                <input type="text" name="period[]" value="{{ old('period.0', isset($newObjective) ? $newObjective->period : '') }}" class="form-control" required>
            </div>
            <div class="col-md-4">
                <label>Type:</label>
                <select class="form-control" name="docutype[]">
                    <option value="-"></option>
                    <option value="Monthly" {{ (isset($newObjective) && $newObjective->docutype == 'Monthly') ? 'selected' : '' }}>Monthly</option>
                    <option value="Semi-annual" {{ (isset($newObjective) && $newObjective->docutype == 'Semi-annual') ? 'selected' : '' }}>Semi-annual</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table id="objectiveTable">
            ...
            <tbody>
                {{-- ตรวจสอบว่ามีข้อมูลจากกิจกรรมเก่า ($objectiveDetails) ส่งมาไหม ถ้ามีให้ลูปแสดงผล --}}
                @if(isset($objectiveDetails) && count($objectiveDetails) > 0)
                    @foreach($objectiveDetails as $i => $detail)
                    <tr>
                        <td>{{ $i+1 }}</td>
                        <td>
                            <textarea name="description[]" rows="3">{{ $detail['description'] ?? '' }}</textarea>
                            สาเหตุ : <input class="form-control" name="note1[]" value="{{ $detail['note1'] ?? '' }}">
                            แนวทางแก้ไข : <input class="form-control" name="note2[]" value="{{ $detail['note2'] ?? '' }}">
                        </td>
                        <td>
                            <select class="form-control receiver-select" name="resp_person[]">
                                <option value=""></option>
                                @foreach ($emp as $item)
                                     <option value="{{ $item->ms_employee_fullname }}" {{ (isset($detail['resp_person']) && $detail['resp_person'] == $item->ms_employee_fullname) ? 'selected' : '' }}>
                            ...
                            <select class="form-control receiver-select" name="resp_person1[]">
                                <option value=""></option>
                                @foreach ($emp as $item)
                                     <option value="{{ $item->ms_employee_fullname }}" {{ (isset($detail['resp_person1']) && $detail['resp_person1'] == $item->ms_employee_fullname) ? 'selected' : '' }}>
                                         {{ $item->ms_employee_fullname }}
                                     </option>
                                @endforeach
                            </select>
                        ...
                        <td><input type="text" name="previous[]" placeholder="Previous" value="{{ $detail['previous'] ?? '' }}"></td>
                        <td><input type="text" name="plan[]" placeholder="Plan" value="{{ $detail['plan'] ?? '' }}"></td>
                        <td><input type="text" name="results[]" placeholder="Results" value="{{ $detail['results'] ?? '' }}"></td>
                        <td>
                    ...
                    @endforeach
                @else
                    {{-- กรณีสร้างใหม่ปกติ แสดง 5 แถวว่าง --}}
                    @for($i = 0; $i < 5; $i++)
                    <tr>
                        <td>{{ $i+1 }}</td>
                        <td>
                        ...
                        <td>
                            <textarea name="remarks[]" rows="3">{{ trim(old('remarks.'.$i)) }}</textarea>
                        </td>
                        <td>
                            <input type="hidden" name="no[]" value="{{ $i+1 }}" class="rowNo">
                            <button type="button" class="delete"><i class="fas fa-trash-alt"></i> ลบ</button>
