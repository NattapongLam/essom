...
@media (max-width: 768px) {
  ...
  input[type="text"], input[type="date"] {
    ...
    <h2>รายละเอียดข้อมูลอีเมล</h2>
    <div class="form-container">
    <table>
        <tr><th>Email Account</th><td>{{ $record->email_account }}</td></tr>
            <tr><th>item</th><td>{{ $record->item}}</td></tr>
        <tr><th>Password</th><td>{{ $record->password }}</td></tr>
        <tr><th>User</th><td>{{ $record->user_name }}</td></tr>
        <tr><th>Position</th><td>{{ $record->position }}</td></tr>
        <tr><th>Department</th><td>{{ $record->department }}</td></tr>
        <tr><th>Approved by</th><td>{{ $record->approved_by }}</td></tr>
        <tr><th>Date</th><td>{{ $record->date }}</td></tr>
        <tr><th>Remark</th><td>{{ $record->remark }}</td></tr>
    </table>

     <div class="actions">
        <a href="{{ route('email-registration.edit', $record->email_id) }}" ><button class="edit">แก้ไข</button></a>
        <a href="{{ route('email-registration.index') }}" ><button  class="primary">กลับ</button> </a>
    </div>
</div>
  </div>
</div>
