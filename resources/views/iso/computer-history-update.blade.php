...
<title>อัปเดตประวัติคอมพิวเตอร์</title>
...
<div class="form-container">
  <h2>อัปเดตประวัติคอมพิวเตอร์</h2>

  <form method="POST" action="{{ route('computer-history.update', $computerHistory->id) }}">
    @csrf
    @method('PUT')
    ...
    <div class="section flex-row">
      ...
      <label><input type="checkbox" name="type_pc" {{ old('type_pc', $computerHistory->type_pc) ? 'checked' : '' }}> PC</label>
      <label><input type="checkbox" name="type_notebook" {{ old('type_notebook', $computerHistory->type_notebook) ? 'checked' : '' }}> Notebook</label>
    </div>
    ...
    <div class="section flex-row">
      <label>4) RAM:</label>
      <label><input type="checkbox" name="ram_ddr1" {{ old('ram_ddr1', $computerHistory->ram_ddr1) ? 'checked' : '' }}> DDR I</label>
      <label><input type="checkbox" name="ram_ddr2" {{ old('ram_ddr2', $computerHistory->ram_ddr2) ? 'checked' : '' }}> DDR II</label>
      <label><input type="checkbox" name="ram_ddr3" {{ old('ram_ddr3', $computerHistory->ram_ddr3) ? 'checked' : '' }}> DDR III</label>
    </div>
    <div class="section flex-row">
      <label>DIMM 1:</label>
    ...
    <div class="section flex-row" style="align-items: center; flex-wrap: wrap; gap: 10px;">
      ...
      <label><input type="checkbox" name="hd_ide" {{ old('hd_ide', $computerHistory->hd_ide) ? 'checked' : '' }}> IDE</label>
      <label><input type="checkbox" name="hd_sata" {{ old('hd_sata', $computerHistory->hd_sata) ? 'checked' : '' }}> SATA</label>
      <label><input type="checkbox" name="hd_sas" {{ old('hd_sas', $computerHistory->hd_sas) ? 'checked' : '' }}> SAS</label>
      <label><input type="checkbox" name="hd_other" {{ old('hd_other', $computerHistory->hd_other) ? 'checked' : '' }}> Other</label>
      <label>Qty:</label>
    ...
    <div class="section flex-row">
      ...
      <label><input type="checkbox" name="cd_ide" {{ old('cd_ide', $computerHistory->cd_ide) ? 'checked' : '' }}> IDE</label>
      <label><input type="checkbox" name="cd_sata" {{ old('cd_sata', $computerHistory->cd_sata) ? 'checked' : '' }}> SATA</label>
      <label>Qty:</label>
    ...
    <div class="section flex-row">
      <label>8) VGA:</label>
      <label><input type="checkbox" name="vga_onboard" {{ old('vga_onboard', $computerHistory->vga_onboard) ? 'checked' : '' }}> Onboard</label>
      <label><input type="checkbox" name="vga_display" {{ old('vga_display', $computerHistory->vga_display) ? 'checked' : '' }}> Display Card</label>
      <label><input type="checkbox" name="vga_pci" {{ old('vga_pci', $computerHistory->vga_pci) ? 'checked' : '' }}> PCI</label>
      <label><input type="checkbox" name="vga_pcie" {{ old('vga_pcie', $computerHistory->vga_pcie) ? 'checked' : '' }}> PCIE</label>
    ...
    <div class="section flex-row">
      ...
      <label><input type="checkbox" name="lan_onboard" {{ old('lan_onboard', $computerHistory->lan_onboard) ? 'checked' : '' }}> Onboard</label>
      <label><input type="checkbox" name="lan_usb" {{ old('lan_usb', $computerHistory->lan_usb) ? 'checked' : '' }}> USB Wireless</label>
      <label><input type="checkbox" name="lan_card" {{ old('lan_card', $computerHistory->lan_card) ? 'checked' : '' }}> LAN Card</label>
      <label><input type="checkbox" name="lan_pci" {{ old('lan_pci', $computerHistory->lan_pci) ? 'checked' : '' }}> PCI</label>
      <label><input type="checkbox" name="lan_pcie" {{ old('lan_pcie', $computerHistory->lan_pcie) ? 'checked' : '' }}> PCIE</label>
      <label>Brand / Spec:</label>
    ...
    <div class="section flex-row">
      ...
      <label><input type="checkbox" name="psu_ide" {{ old('psu_ide', $computerHistory->psu_ide) ? 'checked' : '' }}> IDE Plug</label>
      <label><input type="checkbox" name="psu_sata" {{ old('psu_sata', $computerHistory->psu_sata) ? 'checked' : '' }}> SATA Plug</label>
      <label>Watt:</label>
  ...
  <div class="flex-row">
    ...
    <input type="text" name="ack_by" value="{{ old('ack_by', $computerHistory->ack_by) }}">
    <label>Date:</label>
    <input type="date" name="ack_date" value="{{ old('ack_date', $computerHistory->ack_date) }}">
  </div>
</div>
