...
<style>
    /* Custom Modern Indigo Theme Styles for Edit Form */
    .custom-card {
            ...
            <div class="card custom-card">
                ...
                <div class="custom-card-header d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                    <div class="text-center text-md-left">
                        ...
                        <div class="doc-meta opacity-75">แก้ไขการทบทวนการออกแบบ / EDIT DESIGN REVIEW</div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group-title">
                                    <i class="fas fa-edit me-2 text-indigo"></i> 1. Design Review (แก้ไขข้อมูล)
                                </div>
                            </div>
                        </div> 
                                    ...
                                    <table class="table modern-table-form text-center w-100 mb-0" id="destroyTable">
                                        ...
                                        <tbody>
                                            @foreach ($dt as $index => $item)
                                                <tr>
                                                    <td>
                                                        <input type="hidden" name="listno[]" value="{{ $index + 1 }}">
                                                        <input type="text" class="form-control form-control-modern" name="design_review_a_dt_item[]" value="{{$item->design_review_a_dt_item}}">
                                                    </td>
                                                    <td>
                                                        <input type="text" class="form-control form-control-modern" name="design_review_a_dt_description[]" value="{{$item->design_review_a_dt_description}}">
                                                    </td>
                                                    <td class="text-center">
                                                        <button type="button" class="btn btn-sm btn-delete-row px-3" onclick="confirmDel('{{ $item->design_review_a_dt_id }}', this)">
                        ...
                        <div class="row mt-4">
                            <div class="col-12 d-flex justify-content-end">
                                <button type="submit" class="btn btn-indigo-action px-5 py-2.5 w-100 w-md-auto">
                                    <i class="fas fa-save me-2"></i> อัปเดตข้อมูลเอกสาร
                                </button>
                            </div>
                        </div>
                    </form>
...
// ✅ ฟังก์ชันลบแถวที่เพิ่งเพิ่ม (ยังไม่ได้บันทึกลง Database)
...
// ✅ ฟังก์ชันลบข้อมูลจาก Database ผ่าน AJAX (สำหรับแถวเดิมที่มีอยู่แล้ว)
confirmDel = (refid, element) => {       
    Swal.fire({
        title: 'คุณแน่ใจหรือไม่ !',
        text: `คุณต้องการลบรายการย่อยนี้ออกจากระบบใช่หรือไม่ ?`,
        icon: 'warning',
                ...
                success: function(data) {
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            // ลบแถวออกจากหน้าจอทันทีโดยไม่ต้อง Refresh หน้าทั้งหมด
