...
<div class="container-fluid">
    <div class="card custom-card">
        ...
        <div class="card-body" style="padding: 2rem 2.5rem;">
            <form method="POST" action="{{ route('product-registration.update', $hd->product_registration_hd_id) }}" enctype="multipart/form-data">
                @csrf       
                @method('PUT')      
                ...
                <div class="form-section-panel">
                    <div class="row">
                        <div class="col-md-9 mb-3 mb-md-0">
                            ...
                            <input type="text" class="form-control-modern" name="product_registration_hd_name" value="{{ $hd->product_registration_hd_name }}">
                        </div>
                        <div class="col-md-3">
                            <label for="product_registration_hd_subcode">Sub Code</label>
                            <input type="text" class="form-control-modern" name="product_registration_hd_subcode" value="{{ $hd->product_registration_hd_subcode }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div style="font-weight: 700; color: #1e293b; font-size: 1rem;">
                        <i class="fas fa-edit text-indigo mr-1" style="color: #4f46e5;"></i> แก้ไขรายการแบบผลิต (Edit Drawing List)
                    </div>
                            ...
                            @foreach ($dt as $item)
                                <tr>
                                    <td class="font-weight-bold text-secondary">
                                        <span class="row-number">{{ $item->product_registration_dt_listno }}</span>
                                        <input type="hidden" name="product_registration_dt_id[]" value="{{ $item->product_registration_dt_id }}">
                                        <input type="hidden" name="product_registration_dt_listno[]" value="{{ $item->product_registration_dt_listno }}">            
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern" name="product_registration_dt_dwgno[]" value="{{ $item->product_registration_dt_dwgno }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern" name="product_registration_dt_description[]" value="{{ $item->product_registration_dt_description }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev00[]" value="{{ $item->product_registration_dt_rev00 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev01[]" value="{{ $item->product_registration_dt_rev01 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev02[]" value="{{ $item->product_registration_dt_rev02 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev03[]" value="{{ $item->product_registration_dt_rev03 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev04[]" value="{{ $item->product_registration_dt_rev04 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev05[]" value="{{ $item->product_registration_dt_rev05 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev06[]" value="{{ $item->product_registration_dt_rev06 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev07[]" value="{{ $item->product_registration_dt_rev07 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev08[]" value="{{ $item->product_registration_dt_rev08 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev09[]" value="{{ $item->product_registration_dt_rev09 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control-modern text-center p-1" name="product_registration_dt_rev10[]" value="{{ $item->product_registration_dt_rev10 }}">
                                    </td>
                                    <td>
                                        <a href="javascript:void(0)" class="btn-table-delete" onclick="confirmDel('{{ $item->product_registration_dt_id }}', this)">
                ...
                <div class="d-flex justify-content-start mt-4">
                    <button type="submit" class="btn-indigo-submit">
                        <i class="fas fa-save mr-2"></i> บันทึกการแก้ไขเอกสาร
                    </button>
                </div>
            </form>  
        </div>
...
<script>
// ✅ ฟังก์ชันเพิ่มแถวใหม่ (สำหรับแถวที่ยังไม่มีในฐานข้อมูล จะไม่มี input id)
function addRow() {
    ...
    row.innerHTML = `
        <td class="font-weight-bold text-secondary">
            ...
            <input type="hidden" name="product_registration_dt_id[]" value="">
...
// ✅ ฟังก์ชันลบแถวแบบธรรมดา (ใช้กับแถวใหม่ที่เพิ่มเข้ามาเองแต่ยังไม่ได้บันทึก)
...
// ✅ ฟังก์ชันจัดระเบียบและเรียงลำดับตัวเลขแถวใหม่
...
// ✅ ฟังก์ชันลบแถวเดิมที่มีในระบบผ่าน Ajax พร้อม SweetAlert2 โทน Indigo
confirmDel = (refid, element) => {       
    // หากเป็นแถวที่เพิ่มมาใหม่และยังไม่มี refid ในเบส ให้หักแถวออกได้ทันที
    if(!refid || refid === "") {
        removeRow(element);
        return;
    }
    ...
    Swal.fire({
        ...
        text: 'คุณต้องการลบรายการย่อยนี้ออกจากระบบหรือไม่ ?',
        icon: 'warning',
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.status == true) {
                        Swal.fire({
                            title: 'สำเร็จ',
                            text: 'ลบรายการย่อยเรียบร้อยแล้ว',
                            icon: 'success',
                            confirmButtonColor: '#4f46e5'
                        }).then(function() {
                            // ลบองค์ประกอบแถวออกจากหน้าจอโดยไม่ต้องรีโหลดหน้าทั้งหมด
